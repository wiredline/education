...
class JsonParse
{
    ...
    void Parse(const std::string& input){
        ...
        try{
            ...
            if (!j.contains("operation") || !j.contains("args")) throw std::invalid_argument("Missing required fields");
...
class DataCalculate
{
  ...
  static std::unique_ptr<math::BaseMathClass> Create(const std::string& op) {
    if (op == "+") return std::make_unique<math::SumMath>();
    if (op == "-") return std::make_unique<math::SubMath>();
    if (op == "*") return std::make_unique<math::MulMath>();
    if (op == "/") return std::make_unique<math::DivMath>();
    if (op == "^") return std::make_unique<math::MaxMath>();
    if (op == "!") return std::make_unique<math::FactMath>();
...
int main(int argc, char *argv[]) //'{"operation": "+", "args": [3, 5]}'
{
    ...
    if(argc < 2) throw std::runtime_error("not loaded json");
    //start
