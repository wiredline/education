...
TEST(JsonParseTest, InvalidJsonFormat) {
  ...
  EXPECT_THROW(parser.Parse("{operation: +}"), std::exception);
}

TEST(JsonParseTest, MissingFields) {
  JsonParse parser;

  EXPECT_THROW(parser.Parse(R"({"operation": "+"})"), std::exception);
}
...
TEST(JsonCheckTest, ValidAddition) {
  ...
  EXPECT_NO_THROW(checker.Validate("+", {3, 5}));
}

TEST(JsonCheckTest, InvalidOperation) {
  JsonCheck checker;

  EXPECT_THROW(checker.Validate("%", {3, 5}), std::invalid_argument);
}

TEST(JsonCheckTest, WrongArgsCount) {
  JsonCheck checker;

  EXPECT_THROW(checker.Validate("+", {1}), std::invalid_argument);
}

TEST(JsonCheckTest, DivisionByZero) {
  JsonCheck checker;

  EXPECT_THROW(checker.Validate("/", {10, 0}), std::runtime_error);
}

TEST(JsonCheckTest, FactorialNegative) {
  JsonCheck checker;

  EXPECT_THROW(checker.Validate("!", {-5}), std::invalid_argument);
}
...
TEST(DataCalculate, UnknownOperation) {
  EXPECT_THROW(DataCalculate::Create("%"), std::invalid_argument);
}