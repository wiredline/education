...
public:
  explicit Runner(const std::string &input) : m_input(input) {
    m_db = std::make_unique<PgClient>("host=localhost port=5432 dbname=calc_db "
                                      "user=calc_user password=P@ssw0rd");
  ...
  static bool helper(int argc, char *argv[]) {
    ...
    if (argc == 1 || (argc > 1 && std::string(argv[1]) == "-h")) {
      ...
      std::cout
          << "Can addition(+), subtraction(-), multiplication(*), division(/), "
  ...
  void WarmUpCache() {
    ...
    Logger::Instance().LogInfo("cache loaded with " +
